...
export default async (src: string, targetDir: string, settings: Settings = {}): Promise<void> => {
  ...
  settings.renditions.forEach(rendition => {
    ...
    renditionsCmd += ` -c:a ${settings.audioCodec} -c:v ${settings.videoCodec} -sc_threshold 0 -g ${keyFramesInterval} -keyint_min ${keyFramesInterval} -hls_time ${config.SEGMENT_TARGET_DURATION} -hls_playlist_type vod`;
